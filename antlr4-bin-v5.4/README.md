In this folder you can find a group of useful scripts to increase the productivity of ANTLR4 programming.
...
antlr4-main:
- Creates a java Main class for a grammar.  This command has several options that allows the
  creation of an interactive interpreter (see -i and -is options) and/or the inclusion of
  listeners and/or visitors (execute antlr4-main without arguments to see all the available options).
...
- The current version tries to be smart, and accepts its execution without arguments (or simply with -tokens, -tree,
...
- If you install ANTLR4 documentation (antlr4-4.7.*-javadoc.zip) in /usr/local/doc directory, no change is
